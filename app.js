        ...
        const URL_READ_FLASH = `${BASE_API}/read-flash`;
        const URL_WRITE_FLASH = `${BASE_API}/write-flash`;

        const SCAN_KEY = "SIGD_SECURE_2025"; 
        const URL_REDIRECT_FAILURE = "https://google.com";

                // --- CONFIG SAAS (Géolocalisation dynamique) ---
        const URL_GET_CONFIG = `${BASE_API}/read-config`; // Nouvelle route à créer dans Make
...
function setSession(n, r, id){ 
 ...
 async function refreshAllData() {
    ...
    try {
        // 1. Tâches parallèles non critiques (GPS, Flash, Congés)
        const secondaryTasks = [
            fetchCompanyConfig().catch(e => console.warn("GPS ignoré", e)),
            fetchFlashMessage().catch(e => console.warn("Flash ignoré", e)),
            fetchLeaveRequests().catch(e => console.warn("Congés ignorés", e))
        ];

        // 2. Gestion des Rôles Spéciaux
        if (currentUser) {
            if (['RH', 'ADMIN'].includes(currentUser.role)) {
                secondaryTasks.push(fetchCandidates().catch(e => console.log(e)));
            }
            if (currentUser.role === 'ADMIN') {
                secondaryTasks.push(fetchLogs().catch(e => console.log(e)));
            }
        }

        // 3. CHARGEMENT CRITIQUE : On attend d'avoir les employés AVANT de charger le profil
        await fetchData(true); // <--- Ceci remplit la variable 'employees'

        // 4. Une fois les employés chargés, on affiche le profil
        if (currentUser.role === 'EMPLOYEE') {
            loadMyProfile(); 
        }

        // 5. On lance le reste en arrière-plan
        ...
        Swal.fire('Attention', 'Erreur de connexion partielle.', 'warning');
    } finally {
...
function renderData() { 
            ...
            // 1. CALCUL DES STATISTIQUES ET ALERTES (Sur tout l'effectif)
            employees.forEach(e => { 
                total++; 
                const rawStatus = (e.statut || 'Actif').toLowerCase().trim();
                const isSortie = rawStatus.includes('sortie'); // On identifie le statut Sorti
                
                if (rawStatus === 'actif') actifs++; 
                
                let dL = 999, isU = false, isExpired = false;
                
                if(e.date && !isSortie) { // RÈGLE : On calcule l'alerte pour tout le monde SAUF les Sortis
                    ...
                    // Affichage Dashboard (Alertes) - Si expiré ou urgent et pas Sorti
                    if(isExpired || isU) {
                ...
                text.innerText = "NON POINTÉ"; text.classList.remove('text-emerald-500'); text.classList.add('text-slate-800');
                 ...
                 async function handleClockInOut() {
                    ...
                    Swal.fire({ 
                        ...
                        text: 'Vérification de la zone autorisée...', 
                        didOpen: () => Swal.showLoading(), 
                        allowOutsideClick: false 
                    });

                    try {
                        // 2. Récupération Position Téléphone
                        ...
                        // 3. VÉRIFICATION DE LA ZONE (Si la config SaaS est chargée)
                        // On vérifie si la variable globale companyConfig contient des coordonnées
                        if (typeof companyConfig !== 'undefined' && companyConfig.latitude && companyConfig.longitude) {
                            
                            // Calcul de la distance
                            dist = getDistance(userLat, userLon, companyConfig.latitude, companyConfig.longitude);
                            
                            console.log(`📏 Distance: ${Math.round(dist)}m (Max: ${companyConfig.radius}m)`);

                            // Si la validation stricte est active et qu'on est trop loin
                            if (companyConfig.geo_required && dist > companyConfig.radius) {
                                Swal.fire({
                                    icon: 'error',
                                    title: 'Hors Zone',
                                    html: `Vous êtes à <b>${Math.round(dist)} mètres</b> du bureau.<br>Le pointage est autorisé uniquement dans un rayon de ${companyConfig.radius}m.`,
                                    confirmButtonColor: '#ef4444'
                                });
                                return; // ⛔ STOP : On arrête tout ici, rien n'est envoyé.
                            }
                        }

                    } catch(e) { 
                        console.warn("Erreur GPS:", e.message);
                        // Si la config exige le GPS mais qu'on ne peut pas le récupérer
                        if (typeof companyConfig !== 'undefined' && companyConfig.geo_required) {
                            return Swal.fire('GPS Requis', 'Impossible de valider votre position. Activez le GPS et réessayez.', 'warning');
                        }
                        locationCoords = "Erreur/Désactivé";
                    }
                    ...
                    const payload = {
                        ...
                        gps: locationCoords, 
                        distance_bureau: Math.round(dist), // On ajoute la distance pour les logs
                        agent: currentUser.nom
                    };

                    // 5. Gestion Hors-Ligne (Stockage local)
                    if (!navigator.onLine) {
                        ...
                        document.getElementById('clock-last-action').innerText = "Dernière action : " + new Date().toLocaleTimeString() + " (En attente réseau)";

                        Swal.fire({ 
                            icon: 'info', title: 'Mode Hors-Ligne', 
                            text: 'Pointage enregistré. Il sera envoyé dès le retour de la connexion.',
                            confirmButtonColor: '#f59e0b'
                        });
                        return;
                    }

                    // 6. Envoi au Serveur (Mode Connecté)
                    try {
                        ...
                        if (response.ok) {
                            ...
                            // Message de succès avec la distance affichée
                            const distMsg = (typeof companyConfig !== 'undefined' && companyConfig.latitude) ? `(à ${Math.round(dist)}m du bureau)` : "";
                            
                            Swal.fire({ 
                                icon: 'success', 
                                title: action === 'CLOCK_IN' ? 'Bienvenue !' : 'Au revoir !', 
                                text: `Pointage validé ${distMsg}`,
    ...
    allDocs.forEach(doc => {
        ...
        dC.innerHTML += `
            <div class="flex items-center justify-between p-3 border border-slate-100 bg-white rounded-xl hover:bg-slate-50 transition-all group mb-3 shadow-sm">
                ...
                <div class="flex gap-2">
                    ${hasLink ? `<button onclick="viewDocument('${doc.link}', '${safeLabel}')" class="p-2 text-blue-600 hover:bg-blue-100 rounded-lg"><i class="fa-solid fa-eye"></i></button>` : ''}
                    
                    <!-- CORRECTION : Le bouton modifier n'apparaît QUE pour la Pièce d'Identité -->
                    ${doc.key === 'id_card' ? `
                ...
                pending.forEach(l => {
                    body.innerHTML += `
                        <tr class="border-b hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-8 py-4 text-xs text-slate-500">${l.debut ? l.debut.toLocaleDateString() : '?'} ➔ ${l.fin ? l.fin.toLocaleDateString() : '?'}</td>
                            <td class="px-8 py-4 text-right flex justify-end gap-2">
                                <button onclick="processLeave('${l.id}', 'Validé')" class="bg-emerald-500 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase">OUI</button>
...
async function fetchFlashMessage() {
    ...
    try {
        ...
        container.innerHTML = ''; // On vide pour le rafraîchissement
        
        const now = new Date().getTime();
        const normalize = (s) => String(s || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").trim();
        const userMe = normalize(currentUser.nom);

        messages.forEach((data, index) => {
            // Nettoyage des données Airtable/Make (Majuscules ou minuscules)
            const clean = (v) => Array.isArray(v) ? v[0] : v;
            const msgText = clean(data.Message || data.message);
            const msgSender = clean(data.Sender || data.sender);
            const msgType = clean(data.Type || data.type) || 'Info';
            const msgDate = clean(data.Date || data.date);
            const msgExpiry = clean(data.Expiration || data.date_expiration);

            if (!msgText) return;

            // 1. NE PAS AFFICHER À SOI-MÊME (On compare Josue Dominique et Josue Dominique)
            if (msgSender && normalize(msgSender) === userMe) {
                console.log("C'est votre propre message, il est masqué pour vous.");
                return;
            }

            // 2. FILTRE EXPIRATION
            if (msgExpiry && now > new Date(msgExpiry).getTime()) {
                console.log("Message expiré, ignoré.");
                return;
            }

            // 3. FILTRE FERMETURE (sessionStorage)
            const msgKey = `flash_closed_${msgDate}_${msgSender}`;
            ...
            const bannerHtml = `
                <div id="flash-msg-${index}" class="${st.bg} rounded-2xl p-4 text-white shadow-lg relative overflow-hidden transition-all duration-500 transform hover:scale-[1.01] animate-fadeIn">
                    <div class="absolute top-0 right-0 p-4 opacity-20"><i class="fa-solid fa-bullhorn text-5xl transform rotate-12"></i></div>
                    <div class="relative z-10 flex items-start gap-4">
                        <div class="p-3 bg-white/20 rounded-xl backdrop-blur-sm"><i class="fa-solid ${st.icon} text-xl animate-pulse"></i></div>
                        <div class="flex-1">
                            <div class="flex justify-between items-start">
                                <p class="text-[9px] font-black uppercase tracking-widest opacity-80 mb-1">${msgType} • PAR ${msgSender.toUpperCase()}</p>
                                <button onclick="closeSpecificFlash('${msgKey}', 'flash-msg-${index}')" class="text-white/60 hover:text-white transition-colors"><i class="fa-solid fa-xmark"></i></button>
                            </div>
                            <p class="font-bold text-sm leading-relaxed">${msgText}</p>
                        </div>
                    </div>
                </div>
            `;
...
// Assure-toi que cette fonction est aussi présente
